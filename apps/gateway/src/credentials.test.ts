...
describe("credential registry + env secret provider", () => {
  ...
  it("secrets registry resolves an env credential and adds it to the known-secret set", () => {
    const ref = storage.credentials.create({ provider: "env", key: "MIX_TEST_TOKEN" });
    const value = secrets.resolve(ref.id);
    expect(value).toBe("super-secret-value");
    expect(secrets.known().has("super-secret-value")).toBe(true);
  });

  it("resolve() throws with a clear message when the env var is missing", () => {
    const ref = storage.credentials.create({ provider: "env", key: "MIX_MISSING_ENV" });
    expect(() => secrets.resolve(ref.id)).toThrow(/env var 'MIX_MISSING_ENV'/);
  });

  it("resolve() throws on unsupported provider (Phase I slice 2)", () => {
    const ref = storage.credentials.create({ provider: "os", key: "anything" });
    expect(() => secrets.resolve(ref.id)).toThrow(/not supported yet/);
  });

  it("secrets.scrub() replaces every known-secret substring", () => {
    const ref = storage.credentials.create({ provider: "env", key: "MIX_TEST_TOKEN" });
    secrets.resolve(ref.id);
