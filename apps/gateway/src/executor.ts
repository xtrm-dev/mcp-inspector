...
/**
 * Register an AbortController for an in-flight call not driven by executeTool()
 * (the tool-call route in routes.ts still owns its own adapter dispatch for
 * renderer + source-hint + traceparent capture; this exposes it to cancel).
 * Returns an unregister function the caller must invoke when the call
 * resolves or rejects, whichever comes first.
 */
export function registerInFlight(executionId: string, controller: AbortController): () => void {
  inFlight.set(executionId, controller);
  return () => {
    if (inFlight.get(executionId) === controller) inFlight.delete(executionId);
  };
}
