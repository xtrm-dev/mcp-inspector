...
// Phase M slice 3: capability -> handler symbol map. Ingest-only contract —
// callers (operator tooling / a future repo indexer) supply the mapping;
// the gateway never resolves symbols itself. FK to source_revision means an
// unknown revision is a hard failure (never silently substitutes "main").
